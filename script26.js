// Elite Fashions Chat Application - Complete Combined Version with ALL Functions
class EliteFashionsChat {
    ...
    renderAdminUsers(snapshot) {
        ...
        // Clear existing users (except admin contact)
        const adminContact = document.getElementById('adminContact');
        userList.innerHTML = '';
        if (adminContact) {
        ...
        snapshot.forEach((userDoc) => {
            ...
            userDiv.style.position = 'relative'; // Needed for red dot positioning

            // Create red dot element
            const unreadDot = document.createElement('div');
            unreadDot.className = 'red-dot';
            ...
            this.checkUnread(userId, unreadDot);
    ...
    checkUnread(userId, unreadDot) {
        try {
            const chatPath = `chat_messages/${userId}/chat`;
            const chatRef = this.firebase.collection(this.db, chatPath);
            const q = this.firebase.query(
                chatRef, 
                this.firebase.where('read', '==', false),
                this.firebase.where('sender', '!=', 'admin')
            );

            this.firebase.onSnapshot(q, (snapshot) => {
                if (snapshot.size > 0) {
    ...
    async startChatWithUser(userId, displayName, email) {
        ...
        this.markMessagesAsRead(userId);
        this.loadUsers(); // Refresh the user list to update red dot
    ...
    renderUserMessages(snapshot) {
        ...
        snapshot.forEach((doc, index) => {
            ...
            if (index === 0) {
                console.log("📌 First message:", message.message);
    ...
    async sendMessage() {
        ...
        // Check for special "&" message and replace it
        if (message === '&') {
            const msgFromURL = new URLSearchParams(window.location.search).get("message");
            if (msgFromURL) {
                message = decodeURIComponent(msgFromURL);
            }
        }       
    ...
    async markMessagesAsRead(userId) {
        try {
            const chatPath = this.isAdmin 
                ? `chat_messages/${userId}/chat` 
                : `chat_messages/${this.currentUser.uid}/chat`;

            const chatRef = this.firebase.collection(this.db, chatPath);
            const q = this.firebase.query(chatRef, this.firebase.where('read', '==', false));
            ...
            snapshot.forEach(docSnap => {
                ...
                const isFromOther = this.isAdmin 
                    ? msg.sender !== 'admin' 
                    : msg.sender !== this.currentUser.uid;

                if (isFromOther) {
                    updates.push(this.firebase.updateDoc(docSnap.ref, { read: true }));
                }
            });
    ...
    renderMessageContent(content) {
        const isImage = content.match(/\.(jpeg|jpg|png|gif|webp|10)$/i) && content.startsWith('http');
        
        if (isImage) {
            return `<img src="${content}" alt="image" style="max-width: 200px; border-radius: 8px;" />`;
        }

        // Handle &message= special redirect URLs
        try {
            if (content.includes('&message=')) {
                const url = new URL(content);
                const msg = url.searchParams.get('message');
                return this.escapeHtml(decodeURIComponent(msg));
            }
        } catch (e) {
            console.warn("⚠️ URL parsing failed:", e);
        }
    ...
    async sendEmail() {
        if (!window.emailJSConfig || !window.emailJSConfig.serviceID) {
            this.showError('EmailJS is not configured properly.');
            return;
        }
    ...
    // Additional helper method for loading users (referenced in some functions)
    loadUsers() {
        if (this.isAdmin) {
            this.loadAdminUsers();
        }
...
window.addEventListener('unhandledrejection', (event) => {
    ...
    if (window.chatApp) {
        window.chatApp.showError('A network error occurred. Please check your connection.');
    }
});